...
func (s *IAMRuleService) ReconcileIAMRoleRule(rule iamRule) (*types.ValidationResult, error) {
	...
	if len(scpFailures) > 0 {
		...
		vr.Condition.Failures = scpFailures
...
func (s *IAMRuleService) ReconcileIAMGroupRule(rule iamRule) (*types.ValidationResult, error) {
	...
	role, err := s.iamSvc.GetGroup(context.Background(), &iam.GetGroupInput{
		GroupName: ptr.Ptr(rule.Name()),
	})

	policyDocs := rule.IAMPolicies()

	scpFailures, err := checkSCP(s.iamSvc, policyDocs, *role.Group.Arn, "role", *role.Group.GroupName)
	if err != nil {
		return vr, err
	}
...
func checkSCP(svc iamApi, policyDocs []v1alpha1.PolicyDocument, policySourceArn string, policySourceType string, policySourceName string) ([]string, error) {
	var scpFailures []string
	var ctxEntry []iamtypes.ContextEntry
	// get list of all actions to be tested
	var actions []string
	...
	sim, err := svc.SimulatePrincipalPolicy(context.Background(), &iam.SimulatePrincipalPolicyInput{
		ActionNames:     actions,
